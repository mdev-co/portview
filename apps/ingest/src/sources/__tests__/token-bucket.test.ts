...
describe('TokenBucket', () => {
  it('starts at full capacity', () => {
    ...
    const bucket = new TokenBucket({ capacity: 200, refillPerSecond: 200, now: clock.now });
  ...
  it('rejects when bucket is empty', () => {
    const clock = makeClock();
    const bucket = new TokenBucket({ capacity: 2, refillPerSecond: 1, now: clock.now });
    expect(bucket.tryConsume()).toBe(true);
    expect(bucket.tryConsume()).toBe(true);
  ...
  it('refills at the configured rate', () => {
    const clock = makeClock();
    const bucket = new TokenBucket({ capacity: 10, refillPerSecond: 100, now: clock.now });
